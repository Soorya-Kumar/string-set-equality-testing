...
def primary(U, m):
    ...
    while True:
        ...
        # ── Receive Round 2 bundle from Secondary ─────────────────────────────
        # Secondary computed K1, picked q itself, and sends everything back.
        # Primary now filters to produce K2 locally — no message sent to Secondary.
        msg = s2p.get()
        ...
        # Cost of Round 2 message (Secondary → Primary):
        # |K1| indices + log2(q) + |K1|*(log2(q) + 2)
...
def secondary(V, m):
    while True:
        ...
        q = random_prime(2 * m + 1, 4 * m, exclude=p)
